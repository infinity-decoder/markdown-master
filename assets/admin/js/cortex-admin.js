// Place at: cortex/assets/admin/js/cortex-admin.js
(function($){
    'use strict';
    ...
    $(document).ready(function(){
        ...
        var $qTable = $('#cortex-questions-table');
        if ( $qTable.length ) {
            var quizId = $qTable.data('quiz-id');

            // Load up-to-date questions via AJAX (refresh)
            function loadQuestions() {
                $.post(Cortex_Admin.ajax_url, {
                    action: 'cortex_get_questions',
                    quiz_id: quizId,
            ...
            function renderQuestions(list) {
                ...
                list.forEach(function(q){
                    ...
                    $row.append('<td><a href="#" class="cortex-edit-question" data-qid="'+ q.id +'">Edit</a> | <a href="#" class="cortex-delete-question" data-qid="'+ q.id +'">Delete</a></td>');
                    $tbody.append( $row );
                });
            }
            ...
            // Show modal
            var $modal = $('#cortex-question-modal');
            var $form = $('#cortex-question-form');

            $('.cortex-add-question-btn').on('click', function(e){
                e.preventDefault();
                openModal('add');
            });

            // Delegate edit
            $qTable.on('click', '.cortex-edit-question', function(e){
                e.preventDefault();
            ...
            // Delegate delete
            $qTable.on('click', '.cortex-delete-question', function(e){
                e.preventDefault();
                var qid = $(this).data('qid');
                if ( ! confirm(Cortex_Admin.strings.confirm_delete_question) ) return;
                $.post(Cortex_Admin.ajax_url, {
                    action: 'cortex_delete_question',
                    question_id: qid,
                    nonce: Cortex_Admin.nonce
                }, function(resp){
                    if ( resp.success ) {
            ...
            function openModal(mode, qid) {
                $('#cortex-modal-title').text( mode === 'edit' ? Cortex_Admin.strings.update_question : Cortex_Admin.strings.add_question );
                $form[0].reset();
                $form.find('input[name="question_id"]').val('');
                // Reset UI rows
                $form.find('.cortex-options-row').show();
                $form.find('.cortex-correct-row').show();
                ...
                if ( mode === 'edit' && qid ) {
                    ...
                    $.post(Cortex_Admin.ajax_url, {
                        ...
                        if ( resp.success ) {
                            ...
                            if ( q ) {
                                populateForm(q);
                                $form.find('input[name="action"]').val('cortex_update_question');
                    ...
                    $form.find('input[name="action"]').val('cortex_add_question');
                    showModal();
                }
            }
            ...
            $('#cortex-modal-cancel').on('click', function(e){
                e.preventDefault();
                hideModal();
            });
            ...
            $form.on('submit', function(e){
                ...
                $.post(Cortex_Admin.ajax_url, data, function(resp){
                    if ( resp.success ) {
                        hideModal();
                        loadQuestions();
                    } else {
