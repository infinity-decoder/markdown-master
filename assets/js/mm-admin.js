...
(function ($) {
    ...
    const MM_QuestionManager = {
        ...
        cacheElements() {
            this.$list = $('#mm-questions-list');
            this.$addBtn = $('#mm-btn-add-question');
            this.$typeSelect = $('#mm-add-question-type');
            this.$form = $('#mm-quiz-editor-form');
            this.$leadBuilder = $('#mm-lead-fields-builder');
        },

        bindEvents() {
            this.$addBtn.on('click', (e) => this.addQuestion(e));
            $(document).on('click', '.mm-toggle-edit', (e) => this.toggleEdit(e));
            $(document).on('click', '.mm-delete-question', (e) => this.deleteQuestion(e));
            $(document).on('click', '.mm-add-option', (e) => this.addOption(e));
            $(document).on('click', '.mm-remove-option', (e) => this.removeOption(e));
            $(document).on('click', '.mm-add-pair', (e) => this.addPair(e));
            $(document).on('click', '.mm-remove-pair', (e) => this.removePair(e));

            $('#mm-btn-add-lead-field').on('click', (e) => this.addLeadField(e));
            $(document).on('click', '.mm-remove-lead-field', (e) => this.removeLeadField(e));

            // Tab system
            $('.mm-tabs-nav a').on('click', function (e) {
                e.preventDefault();
                const target = $(this).attr('href');
                $('.mm-tabs-nav a').removeClass('active');
        ...
        addLeadField(e) {
            e.preventDefault();
            const index = this.$leadBuilder.find('.mm-lead-field-row').length;
            const html = `
                <div class="mm-lead-field-row card">
                    <div class="mm-form-group-inline">
                        <label>Label</label>
                        <input type="text" name="lead_fields[${index}][label]" placeholder="e.g. Class" class="mm-input-medium">
                    </div>
                    <div class="mm-form-group-inline">
                        <label>Type</label>
                        <select name="lead_fields[${index}][type]" class="mm-select-small">
                    ...
                    <button type="button" class="mm-remove-lead-field dashicons dashicons-trash danger"></button>
                </div>
            `;
        ...
        removeLeadField(e) {
            $(e.currentTarget).closest('.mm-lead-field-row').remove();
        },

        initSortable() {
            if (this.$list.length && $.fn.sortable) {
                this.$list.sortable({
                    handle: '.mm-drag-handle',
        ...
        loadQuestions() {
            if (!MM_Admin.quiz_id) return;

            wp.ajax.send('mm_get_questions', {
                data: {
                    quiz_id: MM_Admin.quiz_id,
        ...
        renderQuestion(data) {
            const template = wp.template('mm-question-item');
        ...
        addQuestion(e) {
            ...
            wp.ajax.send('mm_add_question', {
                ...
                success: (data) => {
                    ...
                    $item.find('.mm-toggle-edit').trigger('click');
        ...
        toggleEdit(e) {
            const $item = $(e.currentTarget).closest('.mm-question-item');
            const $body = $item.find('.mm-question-edit-body');
        ...
        saveQuestion($item) {
            ...
            // Options
            const options = [];
            $item.find('.mm-option-row input[type="text"]').each(function () {
            ...
            if (data.type === 'fill_blank') {
                ...
                $item.find('.mm-option-row input[name$="[correct][]"]:checked').each(function () {
            ...
            if (data.type === 'matching') {
                const pairs = {};
                $item.find('.mm-pair-row').each(function () {
            ...
            $('.mm-save-status').text(MM_Admin.strings.saving);

            wp.ajax.send('mm_update_question', {
                data: data,
                success: (q) => {
                    $item.find('.mm-question-title-preview').text(q.question_text || '(Empty Question)');
                    $('.mm-save-status').text(MM_Admin.strings.saved);
                    setTimeout(() => $('.mm-save-status').text(''), 2000);
                },
                error: () => {
                    $('.mm-save-status').text(MM_Admin.strings.error);
                }
            });
        },
        ...
        deleteQuestion(e) {
            ...
            wp.ajax.send('mm_delete_question', {
                data: {
                    id: id,
                    nonce: MM_Admin.nonce
                },
        ...
        addOption(e) {
            const $list = $(e.currentTarget).prev('.mm-options-list');
            const type = $(e.currentTarget).closest('.mm-question-item').data('type');
            const inputType = (type === 'checkbox') ? 'checkbox' : 'radio';
            const index = $list.find('.mm-option-row').length;

            const html = `
                <div class="mm-option-row">
                    <input type="text" name="options[]" value="" class="mm-input-small" placeholder="Option text...">
        ...
        removeOption(e) {
            $(e.currentTarget).closest('.mm-option-row').remove();
        },

        addPair(e) {
            const $list = $(e.currentTarget).prev('.mm-pairs-list');
            const html = `
        ...
        removePair(e) {
            $(e.currentTarget).closest('.mm-pair-row').remove();
        },
