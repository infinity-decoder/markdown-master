<?php
/**
 * Security Layer for Markdown Master
 * 
...
class MM_Security {
    ...
    public static function verify_nonce_or_die( $action, $nonce_field = '_wpnonce', $query_arg = '_wpnonce' ) {
        ...
        if ( ! wp_verify_nonce( $nonce, $action ) ) {
            wp_die(
                esc_html__( 'Security check failed. Please refresh and try again.', 'markdown-master' ),
                esc_html__( 'Security Error', 'markdown-master' ),
                array( 'response' => 403 )
            );
        }
    ...
    public static function check_permission_or_die( $capability, $message = '' ) {
        if ( ! current_user_can( $capability ) ) {
            $error_message = $message ? $message : __( 'You do not have permission to perform this action.', 'markdown-master' );
            
            wp_die(
                esc_html( $error_message ),
                esc_html__( 'Permission Denied', 'markdown-master' ),
                array( 'response' => 403 )
            );
        }
    ...
    public static function check_rate_limit( $key, $limit = 5, $window = 3600 ) {
        $transient_key = 'mm_rate_' . md5( $key );
