...
class MM_Import_Export {
    ...
    public function __construct() {
        ...
        add_action( 'admin_post_mm_export_results_csv', [ $this, 'handle_export_results_csv_post' ] );
    ...
    public function render_quizzes_import_box() {
        ...
        <div class="notice notice-info" style="padding:16px;">
            <h2 style="margin-top:0;"><?php esc_html_e( 'Import Questions (CSV)', 'markdown-master' ); ?></h2>
            <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" enctype="multipart/form-data">
                ...
                <p>
                    <label><strong><?php esc_html_e( 'Choose CSV file', 'markdown-master' ); ?>:</strong></label>
                ...
                <p class="description">
                    <?php
                    esc_html_e( 'CSV headers supported: question_text, question_type, options (pipe-separated), correct_answer.', 'markdown-master' );
                    ?>
                </p>
                <p>
    ...
    public function handle_import_post() {
        ...
        try {
            ...
            wp_safe_redirect( admin_url( 'admin.php?page=mm_quizzes&mm_msg=' . urlencode( sprintf( __( 'Successfully imported %d questions.', 'markdown-master' ), $count ) ) ) );
            exit;
        } catch ( Exception $e ) {
    ...
    public function handle_export_quiz_post() {
        if ( ! current_user_can( 'manage_options' ) ) wp_die( __( 'Permission denied', 'markdown-master' ) );
    ...
    public function handle_export_all_quizzes_post() {
        if ( ! current_user_can( 'manage_options' ) ) wp_die( __( 'Permission denied', 'markdown-master' ) );
        $filename = 'mm-all-quizzes-' . date( 'Ymd-His' ) . '.csv';
        ...
        global $wpdb;
        $quizzes = $wpdb->get_results( "SELECT id, title FROM {$wpdb->prefix}mm_quizzes" );
    ...
    public function handle_export_results_csv_post() {
        ...
        global $wpdb;
        $attempts = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}mm_attempts WHERE quiz_id = %d", $quiz_id ) );

        $filename = "mm-quiz-{$quiz_id}-results-" . date( 'Ymd-His' ) . '.csv';
        header( 'Content-Type: text/csv' );
        header( 'Content-Disposition: attachment; filename=' . $filename );
        $out = fopen( 'php://output', 'w' );
    ...
    protected function stream_quiz_export( int $quiz_id ) {
        ...
        if ( ! $quiz ) wp_die( __( 'Quiz not found', 'markdown-master' ) );
