<?php
/**
 * Enhanced Quiz Model for Markdown Master 2.0
 * 
...
class MM_Quiz {
     ...
     * Create a quiz
     * 
     * @param array $data Quiz data (sanitized externally via MM_Security)
    ...
    public function create_quiz( array $data ) {
        ...
        // Clear cache
        MM_Cache::invalidate_quiz( $quiz_id, $quiz_uuid );
    ...
    public function update_quiz( $id, array $data ) {
        ...
        MM_Cache::invalidate_quiz( $id, $quiz_uuid );
    ...
    public function delete_quiz( $id ) {
        ...
        $table_leads = $this->db->prefix . 'mm_lead_captures';
    ...
    public function get_quiz( $identifier, $with_questions = false ) {
        ...
        if ( is_numeric( $identifier ) ) {
            ...
            $cache_key = MM_Cache::get_quiz_key( $id );
            
            $row = MM_Cache::get( $cache_key, MM_Cache::GROUP_QUIZ );
            ...
            if ( false === $row ) {
                ...
                if ( $row ) {
                    MM_Cache::set( $cache_key, $row, MM_Cache::get_ttl(), MM_Cache::GROUP_QUIZ );
                }
            }
        } else {
            // UUID
            $uuid = sanitize_text_field( $identifier );
            $cache_key = MM_Cache::get_quiz_key( $uuid );
    ...
    public function get_questions( $quiz_id, $randomize = false ) {
        ...
        $cache_key = MM_Cache::get_quiz_questions_key( $quiz_id );
        $rows = MM_Cache::get( $cache_key, MM_Cache::GROUP_QUIZ );
        ...
        if ( false === $rows ) {
            ...
            MM_Cache::set( $cache_key, $rows, MM_Cache::get_ttl(), MM_Cache::GROUP_QUIZ );
        }
    ...
    public function add_question( $quiz_id, array $data ) {
        ...
        // Clear cache
        MM_Cache::invalidate_question( $question_id, $quiz_id );
    ...
    public function check_availability( $quiz ) {
        ...
        if ( ! empty( $quiz['scheduled_start'] ) && $now < $quiz['scheduled_start'] ) {
            return array(
                'available' => false,
                'message'   => sprintf(
                    __( 'This quiz will be available starting %s', 'markdown-master' ),
        ...
        if ( ! empty( $quiz['scheduled_end'] ) && $now > $quiz['scheduled_end'] ) {
            return array(
                'available' => false,
                'message'   => __( 'This quiz has ended and is no longer available.', 'markdown-master' ),
            );
        }
    ...
    public function check_attempt_limits( $quiz_id, $quiz, $user_id = 0 ) {
        ...
        if ( $quiz['max_total_attempts'] > 0 ) {
            ...
            if ( $total_attempts >= $quiz['max_total_attempts'] ) {
                return array(
                    'allowed' => false,
                    'message' => __( 'This quiz has reached its maximum number of attempts.', 'markdown-master' ),
                );
            }
        }
        ...
        if ( $quiz['max_user_attempts'] > 0 && $user_id > 0 ) {
            ...
            if ( $user_attempts >= $quiz['max_user_attempts'] ) {
                return array(
                    'allowed' => false,
                    'message' => sprintf(
                        __( 'You have reached the maximum number of attempts (%d) for this quiz.', 'markdown-master' ),
    ...
    public function record_attempt( $quiz_id, array $attempt_data ) {
        ...
        $insert_data = array(
            ...
            'ip_address'      => isset( $attempt_data['ip_address'] ) ? $attempt_data['ip_address'] : MM_Security::get_user_ip(),
            'user_agent'      => isset( $attempt_data['user_agent'] ) ? $attempt_data['user_agent'] : MM_Security::get_user_agent(),
